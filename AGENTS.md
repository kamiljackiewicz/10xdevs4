...
Projekt to aplikacja Astro z komponentami React, TypeScript, Tailwind, Supabase i integracją Cloudflare. Szczegóły produktu oraz konfiguracji lokalnej opisuje @README.md; polecenia i zależności są źródłem prawdy w @package.json.

## Critical Rules

- Zachowuj istniejącą zawartość `context/`, a do `context/archive/` nigdy nie zapisuj. Archiwalne zmiany są niezmienne.
- Nie dodawaj sekretów ani rzeczywistych wartości Supabase do repozytorium. Używaj nazw z @.env.example.
- Przed zakończeniem zmiany uruchom `npm run lint` i `npm run build`. Dla zmian w logowaniu uruchom także działający lokalnie serwer oraz `npm run smoke`.

## Project Structure

- `src/pages/` zawiera strony Astro oraz endpointy API, w tym trasy uwierzytelniania w `src/pages/api/auth/`.
- `src/components/` zawiera komponenty UI; formularze uwierzytelniania znajdują się w `src/components/auth/`, a współdzielone komponenty bazowe w `src/components/ui/`.
- `src/lib/` zawiera integracje i narzędzia współdzielone, `src/styles/` style globalne, a `scripts/` narzędzia uruchamiane poza aplikacją.

## Commands and Quality Checks

- `npm run dev` uruchamia środowisko lokalne; `npm run preview` obsługuje zbudowaną aplikację.
- `npm run lint`, `npm run lint:fix` i `npm run format` sprawdzają lub poprawiają jakość kodu.
- `npm run smoke` sprawdza pełny przepływ auth pod `BASE_URL`, domyślnie `http://localhost:4321`.
- Używaj TypeScript z aliasem `@/` do `src/`. Prettier wymusza dwa spacje, podwójne cudzysłowy, średniki i maksymalnie 120 znaków w wierszu.

## CI and Commits

CI instaluje zależności przez `npm ci`, uruchamia `astro sync`, lint, Astro check, build oraz smoke test z lokalnym Supabase. Hook pre-commit uruchamia `lint-staged`; trzymaj commity krótkie, opisowe i bez wymaganych prefiksów Conventional Commits, zgodnie z istniejącą historią Git.
