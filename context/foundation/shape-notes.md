...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "Observing treatment effects"
      decision: "The application shows observed parameter changes before and after recorded treatment begins and requires medical consultation; it does not make clinical conclusions or suggest treatment changes."
    - topic: "Sharing patient data"
      decision: "Patient-profile sharing was considered, then deferred until after the MVP."
    - topic: "Login and role details"
      decision: "The MVP requires login to access a caregiver's own patient profile; the specific login method and detailed permissions are not yet important."
    - topic: "First MVP flow"
      decision: "AI extracts a single result PDF; the caregiver verifies the extraction, saves selected parameters, and views their changes against manually recorded dated notes."
    - topic: "Patient-profile sharing in the MVP"
      decision: "Adding caregivers is deferred until after the MVP; the MVP focuses on one caregiver and one patient."
  frs_drafted: 7
...
The product is intended for a small user group; increasing its user scale does not change the result-comparison rule.
...
- Timeline-cost acknowledged: present; the scoped first flow is one week.
- Non-Goals: present.
